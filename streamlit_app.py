...
STEP_NAMES = ["Dane jednostki", "Ksiegi rachunkowe", "Metody wyceny", "Koszty i RZiS",
...
# ══════════════════════════════════════════════════════
# KRS API - DOKLADNIE JAK W INFORMACJI DODATKOWEJ
# ══════════════════════════════════════════════════════
...
def _parse_odpis(data, krs_nr=""):
    try:
        ...
        forma_key = ("sp_zoo" if "ograniczon" in fl else
        ...
        if sklad:
            ...
            nz = nazwisko_obj.get("nazwiskoICzlon", "") if isinstance(nazwisko_obj, dict) else str(nazwisko_obj)
            im = imiona_obj.get("imie", "") if isinstance(imiona_obj, dict) else str(imiona_obj)
        ...
        return {
            ...
            "krs": krs_val, "regon": regon_val, "forma_key": forma_key,
            "forma_prawna": forma, "rep": rep,
        }
    except Exception:
        return None
...
# ══════════════════════════════════════════════════════
# SIDEBAR — DANE JEDNOSTKI + KRS (jak w Informacji Dodatkowej)
# ══════════════════════════════════════════════════════
...
with st.sidebar:
    ...
    krs_input = st.text_input(
        "Numer KRS spolki",
        placeholder="0000640431",
        help="Wpisz 10-cyfrowy numer KRS."
    )
            ...
            with st.spinner("Pobieranie z API KRS..."):
                try:
                    ...
                    if krs_data:
                        ...
                        st.error("Nie znaleziono. Sprawdz numer KRS.")
                except ConnectionError:
                    st.error("Brak polaczenia z API KRS.")
                except TimeoutError:
                    st.error("API KRS nie odpowiada.")
                except Exception as e:
                    st.error(f"Blad: {e}")
        else:
    ...
    if krs.get("forma_key") and krs["forma_key"] in ENTITY_FORM_KEYS:
        default_form_idx = ENTITY_FORM_KEYS.index(krs["forma_key"])
    else:
        default_form_idx = G("d_form") if isinstance(G("d_form"), int) else 0
    fv = st.selectbox("Forma prawna", ENTITY_FORM_LABELS, index=default_form_idx)
...
def gen_docx():
    ...
    for lv, (sz, cl) in {0: (16, "1A3C5E"), 1: (13, "2B5E8C"), 2: (11, "3B6B4F")}.items():
        ...
        h.paragraph_format.space_before = Pt(18 if lv == 0 else 12)
        h.paragraph_format.space_after = Pt(8)

    # Header
    hp = sec.header.paragraphs[0] if sec.header.paragraphs else sec.header.add_paragraph()
    hp.alignment = WD_ALIGN_PARAGRAPH.RIGHT
    hr = hp.add_run(f"Polityka Rachunkowosci - {G('d_name')}")
    hr.font.size = Pt(8)
    hr.font.color.rgb = RGBColor(153, 153, 153)
    hr.font.italic = True
    ...
    rf.font.size = Pt(8)
    rf.font.color.rgb = RGBColor(153, 153, 153)
    rp = fp.add_run()
    rp.font.size = Pt(8)
    ...
    if G("d_small"):
        P("5. Jednostka mala (art. 3 ust. 1c UoR) - uproszczenia.")
    elif G("d_micro"):
        P("5. Jednostka mikro (art. 3 ust. 1a UoR) - uproszczenia.")
    else:
        P("5. Pelne zasady rachunkowosci.")
    ...
    if "Zespol 5" in G("d_cm") or "4 + 5" in G("d_cm"):
        ...
        ohm = {"Klucz przychodowy": "kluczem przychodowym", "Klucz kosztowy": "kluczem kosztowym",
               "Bezposrednie przypisanie": "bezposrednim przypisaniem"}
        P(f"4. Koszty posrednie: {ohm.get(G('d_oh'), G('d_oh'))}.")
    else:
        P("3. Koszty w ukladzie rodzajowym.")
    ...
    if G("d_hfx"):
        ...
        if isinstance(cur, list) and cur:
            P(f"5. Rachunki walutowe: {', '.join(cur)}.")
    else:
        P("4. Brak odrebnych rachunkow walutowych.")

    # VI
    doc.add_heading("VI. Ochrona danych", level=1)
    dpm = {"Elektroniczna i fizyczna": "elektroniczna i fizyczna",
           "Wylacznie elektroniczna": "wylacznie elektroniczna",
           "Wylacznie fizyczna": "wylacznie fizyczna"}
    ...
    if G("d_prov"):
        P("3. Rezerwy na znane ryzyko (art. 35d). RMK wg art. 39.")
    if G("d_dt"):
        if G("d_small") or G("d_micro"):
            P("4. Zaniechanie odroczonego podatku (art. 37 ust. 10).")
        else:
            P("4. Ustalanie aktywow/rezerw odroczonego podatku (art. 37).")
    if G("d_small") or G("d_micro"):
        P("5. Zwolnienie z rachunku przeplywow.")
    else:
    ...
    P("4. Zalaczniki:")
    doc.add_paragraph("Zal. 1: Zakladowy Plan Kont", style="List Bullet")
    doc.add_paragraph("Zal. 2: Opis systemu informatycznego", style="List Bullet")
    doc.add_paragraph("Zal. 3: System ochrony danych", style="List Bullet")
    doc.add_paragraph()
...
# ══════════════════════════════════════════════════════
# WIZARD STEPS (bez step_0 - dane sa w sidebarze)
# ══════════════════════════════════════════════════════

def step_0():
    st.subheader("Krok 1: Sprawdz dane jednostki")
    st.info("Dane jednostki wypelnij w **panelu bocznym po lewej stronie**. Mozesz tez pobrac je automatycznie z KRS.")
    krs = st.session_state.get("krs_data", {})
    if krs.get("nazwa"):
        st.success(f"Dane pobrane z KRS: **{krs['nazwa']}**")
    ...
    st.write(f"Rok obrotowy: {G('d_fys')} - {G('d_fye')}")
...
def step_3():
    ...
    if "Zespol 4" in cm and "5" not in cm:
        st.session_state.d_pl = "Wariant porownawczy"
        st.info("RZiS: **porownawczy** (auto)")
    elif "Zespol 5" in cm and "4" not in cm:
        st.session_state.d_pl = "Wariant kalkulacyjny"
        st.info("RZiS: **kalkulacyjny** (auto)")
    else:
...
def step_6():
    ...
    c1, c2 = st.columns(2)
    with c1:
        st.session_state.d_adate = st.date_input("Data zatwierdzenia", value=G("d_adate"), key="wad")
    with c2:
        st.session_state.d_edate = st.date_input("Data wejscia w zycie", value=G("d_edate"), key="wed")
...
def step_7():
    ...
    st.download_button("Pobierz plik DOCX", buf, fn,
